...
(`ghcr.io/openhands/agent-canvas:sha-8a5a2c0`, from OpenHands/OpenHands
...
- `k8s/values.yaml` — the values for this instance: image tag pinned to
  `sha-8a5a2c0`, skin ref `feature/skin-format`, RBAC in
...
boots on `sha-8a5a2c0` with `config.skin.repo` set and comes up with that
