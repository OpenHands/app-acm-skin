...
(`ghcr.io/openhands/agent-canvas:sha-6ba9bbd`, from OpenHands/OpenHands
...
- `k8s/values.yaml` — the values for this instance: image tag pinned to
  `sha-6ba9bbd`, skin ref `feature/skin-format`, RBAC in
...
boots on `sha-6ba9bbd` with `config.skin.repo` set and comes up with that
