...
(`ghcr.io/openhands/agent-canvas:sha-59161be`, from OpenHands/OpenHands
...
- `k8s/values.yaml` — the values for this instance: image tag pinned to
  `sha-59161be`, skin ref `feature/skin-format`, RBAC in
...
boots on `sha-59161be` with `config.skin.repo` set and comes up with that
